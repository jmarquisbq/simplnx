...
- **Bounded mesh output state**: Triangle connectivity, face labels, and transferred face data are accumulated only for the active Z slice. Vertex coordinates and unique-owner state use disk-backed fixed records behind a bounded page cache, then stream to the vertex and node-type arrays in contiguous batches.
...
The in-core (Direct) variant is intended for datasets that fit in memory. The out-of-core (Scanline) variant avoids singleton accesses to disk-backed cell and mesh arrays and does not retain state proportional to the generated mesh in RAM. Both variants preserve the same traversal, numbering, face-label, transfer, and seeded problem-voxel semantics.
