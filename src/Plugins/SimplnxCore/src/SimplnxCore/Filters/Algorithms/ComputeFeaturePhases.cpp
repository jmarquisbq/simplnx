...
Result<> ComputeFeaturePhases::operator()()
{
  ...
  const usize totalChunks = (totalPoints + k_ChunkTuples - 1) / k_ChunkTuples;

  MessageHelper messageHelper(m_MessageHandler);
  auto progressHelper = messageHelper.createProgressMessageHelper();
  progressHelper.setMaxProgresss(totalChunks);
  progressHelper.setProgressMessageTemplate("Computing feature phases: {:.1f}%");
