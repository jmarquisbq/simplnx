...
Result<> FillBadDataBFS::operator()()
{
  ...
  for(usize i = 0; i < totalPoints; i++)
  {
    ...
    if(!alreadyChecked[i] && featureIdsStore[i] == 0)
    {
      ...
      if(currentVisitedList.size() >= static_cast<usize>(m_InputValues->minAllowedDefectSizeValue))
      {
  ...
  while(count != 0)
  {
    ...
    for(usize i = 0; i < totalPoints; i++)
    {
      ...
      if(featureName < 0)
      {
        ...
        for(int32 j = 0; j < 6; j++)
        {
          const int64 neighborPoint = static_cast<int64>(i) + neighborPoints[j];
          if(j == 0 && zIndex == 0)
          {
            continue;
          }
