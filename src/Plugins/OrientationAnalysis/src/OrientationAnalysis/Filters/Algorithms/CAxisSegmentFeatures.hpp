 ...
 *
 * Every positive phase that participates in segmentation must be hexagonal
 * (Hexagonal_High 6/mmm or Hexagonal_Low 6/m). Phase-0 and masked-out cells
 * are excluded from validation because they cannot seed or join a feature.
 *
 * ## Connected-Component Labeling
 *
 * Identical to EBSDSegmentFeatures: operator() segments the volume with the
 * base-class connected-component labeling algorithm (SegmentFeatures::executeCCL()),
 * which processes data one Z-slice at a time to keep memory usage bounded.
 *
...
public:
  ...
  /**
   * @brief Executes the c-axis segmentation algorithm using connected-component
   * labeling.
...
protected:
   ...
   * c-axes to belong to the same feature.
   *
