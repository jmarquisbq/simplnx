                           ...
                           usize maximumReadValues, usize maximumWriteValues)
{
  ...
  std::atomic_bool shouldCancel{false};
  IFilter::MessageHandler messageHandler{};
  SIMPLNX_RESULT_REQUIRE_VALID(ApplyContour<int32>(inputStore, outputStore, SizeVec3{dimX, dimY, dimZ}, offsets, predicate, shouldCancel, messageHandler, targetBytes));
...
TEST_CASE("ImageProcessing::ContourEngine: resident state requires a complete dataset-scaled reservation", "[ImageProcessing][ContourEngine][WorkingMemory]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(ImageProcessing::detail::CalculateContourResidentWorkingMemoryBytes<int32>(SizeVec3{std::numeric_limits<usize>::max(), 2, 2}));
...
TEST_CASE("ImageProcessing::ContourEngine: real-OOC selector uses resident state only after a complete grant", "[ImageProcessing][ContourEngine][WorkingMemory]")
{
  ...
  auto run = [&](uint64 budgetBytes) {
    ...
    std::atomic_bool shouldCancel{false};
    IFilter::MessageHandler messageHandler{};
    SIMPLNX_RESULT_REQUIRE_VALID(ApplyContour(inputStore, outputStore, dims, offsets, predicate, shouldCancel, messageHandler));
  ...
  for(const usize dimZ : {usize{2}, usize{7}})
  {
    ...
    for(const bool fullyConnected : {false, true})
    {
      ...
      const auto requireRolling = [&](const std::vector<T>& source, const auto& predicate) {
        ...
        SIMPLNX_RESULT_REQUIRE_VALID(inputStore.copyFromBuffer(0, nonstd::span<const T>(source.data(), source.size())));
        std::atomic_bool shouldCancel{false};
        IFilter::MessageHandler messageHandler{};
        SIMPLNX_RESULT_REQUIRE_VALID(ApplyContour<T>(inputStore, outputStore, dims, offsets, predicate, shouldCancel, messageHandler));

        std::vector<T> actual(source.size());
        SIMPLNX_RESULT_REQUIRE_VALID(outputStore.copyIntoBuffer(0, nonstd::span<T>(actual.data(), actual.size())));
...
TEST_CASE("ImageProcessing::ContourEngine: rolling 3D cancellation after the first plane preserves output", "[ImageProcessing][ContourEngine]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(inputStore.copyFromBuffer(0, nonstd::span<const int32>(input.data(), input.size())));
  IFilter::MessageHandler messageHandler{};
  SIMPLNX_RESULT_REQUIRE_VALID(ApplyContour<int32>(inputStore, outputStore, dims, offsets, predicate, shouldCancel, messageHandler));
...
TEST_CASE("ImageProcessing::RadiusOneStencil2D: planner bounds full-width and overwide typed buffers", "[ImageProcessing][ContourEngine]")
{
  ...
  {
    SIMPLNX_RESULT_REQUIRE_INVALID(CreateRadiusOneStencil2DPlan(/*dimX=*/0, /*dimY=*/7, /*inputValueBytes=*/4, /*outputValueBytes=*/4, /*targetBytes=*/216, false));
    SIMPLNX_RESULT_REQUIRE_INVALID(CreateRadiusOneStencil2DPlan(std::numeric_limits<usize>::max(), /*dimY=*/7, /*inputValueBytes=*/4, /*outputValueBytes=*/4, /*targetBytes=*/216, false));
    SIMPLNX_RESULT_REQUIRE_INVALID(CreateRadiusOneStencil2DPlan(/*dimX=*/9, /*dimY=*/7, /*inputValueBytes=*/4, /*outputValueBytes=*/4, /*targetBytes=*/39, false));
  }
}
