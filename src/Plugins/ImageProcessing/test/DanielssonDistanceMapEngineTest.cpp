...
TEST_CASE("ImageProcessing::DanielssonDistanceMapEngine: hybrid vector-plane router bypasses scratch for resident prefix", "[ImageProcessing][DanielssonDistanceMapEngine][WorkingMemory]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(router.writePlane(0, prefixValues));
  SIMPLNX_RESULT_REQUIRE_VALID(router.writePlane(1, secondPrefixValues));
  SIMPLNX_RESULT_REQUIRE_VALID(router.writePlane(2, suffixValues));

  std::vector<int32> readback(k_VectorValues);
  SIMPLNX_RESULT_REQUIRE_VALID(router.readPlane(0, readback));
  REQUIRE(readback == prefixValues);
  SIMPLNX_RESULT_REQUIRE_VALID(router.readPlane(2, readback));
  REQUIRE(readback == suffixValues);
  SIMPLNX_RESULT_REQUIRE_VALID(router.readPlane(1, readback)); // Prefix/suffix boundary-neighbor routing.
...
TEST_CASE("ImageProcessing::DanielssonDistanceMapEngine: compact vector scratch preserves components and uses the signed-16 boundary", "[ImageProcessing][DanielssonDistanceMapEngine][WorkingMemory]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(router.writePlane(1, values));
  std::vector<int32> readback(k_VectorValues);
  SIMPLNX_RESULT_REQUIRE_VALID(router.readPlane(1, readback));
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(router.writePlane(2, tooLarge));
...
TEST_CASE("ImageProcessing::DanielssonDistanceMapEngine: resident state requires a complete dataset-scaled reservation", "[ImageProcessing][DanielssonDistanceMapEngine][WorkingMemory]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(ImageProcessing::detail::CalculateDanielssonResidentWorkingMemoryBytes<uint8>(SizeVec3{std::numeric_limits<usize>::max(), 2, 2}));
          ...
          "[ImageProcessing][DanielssonDistanceMapEngine][WorkingMemory]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(ImageProcessing::detail::CreateDanielssonResidentVectorPrefixPlan<uint8>(SizeVec3{std::numeric_limits<usize>::max(), 2, 2}, k_GrantBytes));
...
TEST_CASE("ImageProcessing::DanielssonDistanceMapEngine: resident vector prefixes preserve 3D reflective output order", "[ImageProcessing][DanielssonDistanceMapEngine][WorkingMemory]")
{
  ...
  const auto run = [&](usize prefixPlanes, bool squared, bool useSpacing) {
    ...
    SIMPLNX_RESULT_REQUIRE_VALID(outputStore.copyIntoBuffer(0, nonstd::span<float32>(output.data(), output.size())));
    return output;
  };
...
TEST_CASE("ImageProcessing::DanielssonDistanceMapEngine: compact scratch preserves 3D generated-schedule parity at the int16 boundary", "[ImageProcessing][DanielssonDistanceMapEngine][WorkingMemory]")
{
  ...
  DataStore<uint8> inputStore(ShapeType{k_DimZ, k_DimY, k_DimX}, ShapeType{1}, uint8{0});
  SIMPLNX_RESULT_REQUIRE_VALID(inputStore.copyFromBuffer(0, nonstd::span<const uint8>(pattern.data(), pattern.size())));
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(inCore());
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(slab());

  std::vector<float32> inCoreValues(k_VoxelCount);
  std::vector<float32> slabValues(k_VoxelCount);
  SIMPLNX_RESULT_REQUIRE_VALID(inCoreOutput.copyIntoBuffer(0, nonstd::span<float32>(inCoreValues.data(), inCoreValues.size())));
  SIMPLNX_RESULT_REQUIRE_VALID(slabOutput.copyIntoBuffer(0, nonstd::span<float32>(slabValues.data(), slabValues.size())));
