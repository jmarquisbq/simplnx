...
// Builds primary (float32, numComponents) + a uint8 mask (alternating 0/1) under one geometry.
void BuildMaskInputs(DataStructure& ds, usize dim, usize numComponents)
{
  ...
  auto inStore = DataStoreUtilities::CreateDataStore<float32>(ds, DataPath({"Image Geometry", "CellData", "Input"}), cellShape, {numComponents}, IDataAction::Mode::Execute);
  ...
  auto maskStore = DataStoreUtilities::CreateDataStore<uint8>(ds, DataPath({"Image Geometry", "CellData", "Mask"}), cellShape, {1}, IDataAction::Mode::Execute);
  auto* maskArray = DataArray<uint8>::Create(ds, "Mask", maskStore, cellAM->getId());
  auto& maskRef = maskArray->getDataStoreRef();
  for(usize t = 0; t < numTuples; ++t)
  {
    maskRef.setValue(t, static_cast<uint8>(t % 2)); // alternating keep/discard
  }
}

void RunMaskParity(usize numComponents)
{
  UnitTest::LoadPlugins();
  const UnitTest::PreferencesSentinel prefsSentinel(nx::core::DataStorageMode::ForceInCore, 0);
  constexpr usize kDim = 10;
  const float64 outsideValue = -9.0;

  auto run = [&](IFilter& filter, DataStructure& ds) {
    Arguments args;
  ...
  DataStructure newDs;
  BuildMaskInputs(newDs, kDim, numComponents);
  MaskImageFilter newFilter;
  run(newFilter, newDs);

  IFilter::UniquePointer itkFilter = Application::Instance()->getFilterList()->createFilter(*Uuid::FromString("d3138266-3f34-4d6e-8e21-904c94351293"));
  REQUIRE(itkFilter != nullptr);
  DataStructure itkDs;
  BuildMaskInputs(itkDs, kDim, numComponents);
  run(*itkFilter, itkDs);

  UnitTest::CompareDataArrays<float32>(itkDs.getDataRefAs<Float32Array>(DataPath({"Image Geometry", "CellData", "Output"})),
                                       newDs.getDataRefAs<Float32Array>(DataPath({"Image Geometry", "CellData", "Output"})));
}
} // namespace

TEST_CASE("ImageProcessing::MaskImageFilter: Legacy parity (scalar)", "[ImageProcessing][MaskImageFilter]")
{
  RunMaskParity(1);
}

TEST_CASE("ImageProcessing::MaskImageFilter: Legacy parity (3-component vector)", "[ImageProcessing][MaskImageFilter]")
{
  RunMaskParity(3);
}
...
TEST_CASE("ImageProcessing::MaskEngine: adaptive OOC batches read the mask first and honor the live grant", "[ImageProcessing][MaskImageFilter][WorkingMemory]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::ApplyTypedMask(inputStore, outputStore, maskStore, k_NumComponents, int32{-7}, shouldCancel));
...
TEST_CASE("ImageProcessing::MaskEngine: adaptive OOC batches keep multi-component tuples together", "[ImageProcessing][MaskImageFilter][WorkingMemory]")
{
  ...
  const ImageProcessing::ScopedWorkingMemoryTuningOverride workingMemoryOverride(k_GrantedBytes);
  std::atomic_bool shouldCancel{false};
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::ApplyTypedMask(inputStore, outputStore, maskStore, k_NumComponents, int32{-9}, shouldCancel));
...
TEST_CASE("ImageProcessing::MaskEngine: cancellation before a chunk avoids all bulk transfers", "[ImageProcessing][MaskImageFilter][WorkingMemory]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::ApplyTypedMask(inputStore, outputStore, maskStore, /*numComponents=*/1, int32{-7}, shouldCancel));
...
TEST_CASE("ImageProcessing::MaskImageFilter: Preflight guards (Outside Value range + mask components)", "[ImageProcessing][MaskImageFilter]")
{
  ...
  auto buildInt8 = [](DataStructure& ds, usize maskComponents) {
    ...
    auto* cellAM = AttributeMatrix::Create(ds, "CellData", cellShape, imageGeom->getId());
    imageGeom->setCellData(*cellAM);
    auto inStore = DataStoreUtilities::CreateDataStore<int8>(ds, DataPath({"Image Geometry", "CellData", "Input"}), cellShape, {1}, IDataAction::Mode::Execute);
    DataArray<int8>::Create(ds, "Input", inStore, cellAM->getId());
    auto maskStore = DataStoreUtilities::CreateDataStore<uint8>(ds, DataPath({"Image Geometry", "CellData", "Mask"}), cellShape, {maskComponents}, IDataAction::Mode::Execute);
...
void BuildTypedMaskInputs(DataStructure& ds, usize dim, MaskT nonzeroValue)
{
  ...
  const usize numTuples = dim * dim * dim;
  auto inStore = DataStoreUtilities::CreateDataStore<float32>(ds, DataPath({"Image Geometry", "CellData", "Input"}), cellShape, {1}, IDataAction::Mode::Execute);
  auto* inArray = DataArray<float32>::Create(ds, "Input", inStore, cellAM->getId());
  auto& inRef = inArray->getDataStoreRef();
  for(usize i = 0; i < inRef.getSize(); ++i)
  {
  ...
  auto maskStore = DataStoreUtilities::CreateDataStore<MaskT>(ds, DataPath({"Image Geometry", "CellData", "Mask"}), cellShape, {1}, IDataAction::Mode::Execute);
...
TEST_CASE("ImageProcessing::MaskImageFilter: Preflight rejects wrong mask type (-8100) and tuple mismatch (-8101)", "[ImageProcessing][MaskImageFilter]")
{
  ...
  SECTION("wrong mask type (int32) -> -8100")
  {
    ...
    auto maskStore = DataStoreUtilities::CreateDataStore<int32>(ds, DataPath({"Image Geometry", "MaskData", "Mask"}), cellShape, {1}, IDataAction::Mode::Execute);
  ...
  SECTION("mask/input tuple mismatch -> -8101")
  {
    ...
    auto maskStore = DataStoreUtilities::CreateDataStore<uint8>(ds, DataPath({"Image Geometry", "MaskData", "Mask"}), maskShape, {1}, IDataAction::Mode::Execute);
...
// k_MaskImageDataPath_Key, then (B) live-ITK parity FIRST (@p parityTol -- 0 for the integer input, 1e-4 for the float
// input) and (A) md5-validity-first (plan Sec.4). The cthead1 case's primary input is a .mha (MHA reader delivered).
// Pinned ForceInCore: the legacy ITK filter bad_casts an OOC store (reading via OUR reader is OOC-safe).
// -----------------------------------------------------------------------------
void RunMaskItkGolden(const std::string& inputFile, const std::string& maskFile, float64 parityTol, const std::string& committedMd5)
{
  UnitTest::LoadPlugins();
  const UnitTest::PreferencesSentinel prefsSentinel(nx::core::DataStorageMode::ForceInCore, 0);
  ...
  // (B) LIVE-ITK parity: legacy ITK filter on the SAME input+mask into the SAME ds.
  const auto legacyUuid = ip_golden::LegacyUuidFor(FilterTraits<MaskImageFilter>::uuid);
  REQUIRE(legacyUuid.has_value());
  Arguments itkArgs = args;
  itkArgs.insertOrAssign(MaskImageFilter::k_OutputImageArrayName_Key, std::make_any<std::string>("ITK Output"));
...
{
  RunMaskItkGolden("STAPLE1.png", "STAPLE2.png", /*parityTol=*/0.0, "c57d7fda3e42374881c3c3181d15bf90");
}
...
{
  RunMaskItkGolden("cthead1-Float.mha", "cthead1-mask.png", /*parityTol=*/1.0e-4, "0ef8943803bb4a21b2015b53f0164f1c");
}
