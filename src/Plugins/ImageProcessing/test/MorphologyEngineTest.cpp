...
void RequireCropSubtractCase(const SizeVec3& dims, const std::array<usize, 3>& radius, ImageProcessing::detail::CropSubtractOrder order, bool aliasOutput)
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(paddedStore.copyFromBuffer(0, nonstd::span<const T>(paddedValues.data(), paddedValues.size())));
  SIMPLNX_RESULT_REQUIRE_VALID(originalStore.copyFromBuffer(0, nonstd::span<const T>(originalValues.data(), originalValues.size())));
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::detail::CropSubtractFromStore<T>(paddedStore, originalStore, destination, dims, radius, order, shouldCancel));

  std::vector<T> actual(volume);
  SIMPLNX_RESULT_REQUIRE_VALID(destination.copyIntoBuffer(0, nonstd::span<T>(actual.data(), actual.size())));
...
TEST_CASE("ImageProcessing::MorphologyEngine: rolling top-hat pipeline pre-cancel preserves poison output", "[ImageProcessing][MorphologyEngine]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(ApplyMorphologyCompositePipeline<int32>(input, output, dims, se, MorphOp::Erode, MorphOp::Dilate, true,
                                                                       ImageProcessing::detail::MorphologyCompositeOutputMode::OriginalMinusMorphology, shouldCancel, requiredBytesResult.value()));
  std::vector<int32> actual(count);
  SIMPLNX_RESULT_REQUIRE_VALID(output.copyIntoBuffer(0, nonstd::span<int32>(actual.data(), actual.size())));
...
TEMPLATE_TEST_CASE("ImageProcessing::MorphologyEngine: rolling composite pipeline emits morphology and top hats with one read and write", "[ImageProcessing][MorphologyEngine]", int16, float32)
{
  ...
  for(const bool safeBorder : {false, true})
  {
    for(const Case testCase : cases)
    {
      ...
      SIMPLNX_RESULT_REQUIRE_VALID(input.copyFromBuffer(0, values));
      ...
      SIMPLNX_RESULT_REQUIRE_VALID(output.copyIntoBuffer(0, nonstd::span<T>(actual.data(), actual.size())));
      REQUIRE(actual == expected);
...
TEST_CASE("ImageProcessing::MorphologyEngine: rolling top-hat pipeline handles a Z radius larger than the image", "[ImageProcessing][MorphologyEngine]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(requiredBytesResult);
  std::atomic_bool shouldCancel{false};
  SIMPLNX_RESULT_REQUIRE_VALID(ApplyMorphologyCompositePipeline<int32>(input, output, dims, se, MorphOp::Erode, MorphOp::Dilate, false,
                                                                       ImageProcessing::detail::MorphologyCompositeOutputMode::OriginalMinusMorphology, shouldCancel, requiredBytesResult.value()));
...
TEST_CASE("ImageProcessing::MorphologyEngine: OOC scanline slab batches preserve a short tail exactly", "[ImageProcessing][MorphologyEngine]")
{
  ...
  auto inputStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, DataPath({"Input"}), tupleShape, {1}, IDataAction::Mode::Execute);
  auto morphStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, DataPath({"Morph"}), tupleShape, {1}, IDataAction::Mode::Execute);
  auto gradientStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, DataPath({"Gradient"}), tupleShape, {1}, IDataAction::Mode::Execute);
  auto binaryStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, DataPath({"Binary"}), tupleShape, {1}, IDataAction::Mode::Execute);
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::detail::MakeMorphologySlabPlan(*inputStore, *morphStore, SizeVec3{k_DimX, k_DimY, k_DimZ}, 1, k_TargetBytes, plan));
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(morphAlgorithm());
  SIMPLNX_RESULT_REQUIRE_VALID(gradientAlgorithm());
  SIMPLNX_RESULT_REQUIRE_VALID(binaryAlgorithm());
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(morphStore->copyIntoBuffer(0, nonstd::span<int32>(actual.data(), actual.size())));
  REQUIRE(actual == expectedMorph);
  SIMPLNX_RESULT_REQUIRE_VALID(gradientStore->copyIntoBuffer(0, nonstd::span<int32>(actual.data(), actual.size())));
  REQUIRE(actual == expectedGradient);
  SIMPLNX_RESULT_REQUIRE_VALID(binaryStore->copyIntoBuffer(0, nonstd::span<int32>(actual.data(), actual.size())));
...
TEST_CASE("ImageProcessing::MorphologyEngine: BinaryMorphScanline reads each 256-plane input once", "[ImageProcessing][MorphologyEngine]")
{
  ...
  ImageProcessing::detail::MorphologySlabPlan plan;
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::detail::MakeMorphologySlabPlan(inputStore, outputStore, SizeVec3{k_DimX, k_DimY, k_DimZ}, 2, k_TargetBytes, plan));
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(algorithm());

  std::vector<int32> actual(k_TotalValues);
  SIMPLNX_RESULT_REQUIRE_VALID(outputStore.copyIntoBuffer(0, nonstd::span<int32>(actual.data(), actual.size())));
...
TEST_CASE("ImageProcessing::MorphologyEngine: bounded 2D row blocks and overwide tiles match independent oracles", "[ImageProcessing][MorphologyEngine]")
{
  ...
  for(const usize targetBytes : {fullWidthTargetBytes, overwideTargetBytes})
  {
    for(const MorphOp op : {MorphOp::Dilate, MorphOp::Erode})
    {
      ...
      SIMPLNX_RESULT_REQUIRE_VALID(grayscaleAlgorithm());
      std::vector<int32> actual(grayscaleInput.size());
      SIMPLNX_RESULT_REQUIRE_VALID(grayscaleOutput.copyIntoBuffer(0, nonstd::span<int32>(actual.data(), actual.size())));
      ...
      for(const bool boundaryToForeground : {false, true})
      {
        ...
        SIMPLNX_RESULT_REQUIRE_VALID(binaryAlgorithm());
        SIMPLNX_RESULT_REQUIRE_VALID(binaryOutput.copyIntoBuffer(0, nonstd::span<int32>(actual.data(), actual.size())));
...
TEST_CASE("ImageProcessing::MorphologyEngine: bounded gradient 2D transfers never span the complete plane", "[ImageProcessing][MorphologyEngine]")
{
  ...
  for(const TransferCase& transferCase : transferCases)
  {
    DYNAMIC_SECTION(transferCase.label)
    {
      ...
      SIMPLNX_RESULT_REQUIRE_VALID(gradient());
...
TEST_CASE("ImageProcessing::MorphologyEngine: slab planner validates dimensions and store sizes", "[ImageProcessing][MorphologyEngine]")
{
  ...
  if constexpr(std::numeric_limits<usize>::max() > static_cast<usize>(std::numeric_limits<int64>::max()))
  {
    ...
    EmptyDataStore<int32> hugeStore({k_Int64Max}, {1});
...
TEST_CASE("ImageProcessing::MorphologyEngine: slab planner bounds normal batches and retains required large-radius halo", "[ImageProcessing][MorphologyEngine]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::detail::MakeMorphologySlabPlan(inputStore, outputStore, SizeVec3{k_DimX, k_DimY, k_DimZ}, 1, k_HappyTargetBytes, plan));
  REQUIRE(plan.outputBatchDepth == 3);
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::detail::MakeMorphologySlabPlan(inputStore, outputStore, SizeVec3{k_DimX, k_DimY, k_DimZ}, 1, k_NormalizedTargetBytes, plan));
  REQUIRE(plan.outputBatchDepth == 4);
  REQUIRE(plan.maxInputDepth == 6);

  constexpr usize k_OnePlaneTargetBytes = 2 * k_PlaneBytes;
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::detail::MakeMorphologySlabPlan(inputStore, outputStore, SizeVec3{k_DimX, k_DimY, k_DimZ}, 1, k_OnePlaneTargetBytes, plan));
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::detail::MakeMorphologySlabPlan(shallowInput, shallowOutput, SizeVec3{k_DimX, k_DimY, k_LargeRadiusDimZ}, 10, k_LargeRadiusTargetBytes, plan));
  REQUIRE(plan.outputBatchDepth == 1);
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(shallowInput.copyFromBuffer(0, nonstd::span<const int32>(input.data(), input.size())));
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(shallowOutput.copyIntoBuffer(0, nonstd::span<int32>(actual.data(), actual.size())));
...
TEST_CASE("ImageProcessing::MorphologyEngine: resident 8-bit plan requires complete dataset-scaled input and output memory", "[ImageProcessing][MorphologyEngine][WorkingMemory]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(ImageProcessing::detail::CalculateMorphologyResidentWorkingMemoryBytes<uint8>(SizeVec3{std::numeric_limits<usize>::max(), 2, 1}));
...
TEST_CASE("ImageProcessing::MorphologyEngine: pre-cancel preserves poisoned scanline outputs", "[ImageProcessing][MorphologyEngine]")
{
  ...
  {
    SIMPLNX_RESULT_REQUIRE_VALID(store->copyIntoBuffer(0, nonstd::span<int32>(actual.data(), actual.size())));
...
TEST_CASE("ImageProcessing::MorphologyEngine: fused SafeBorder crop-subtract uses registered OOC stores", "[ImageProcessing][MorphologyEngine]")
{
  ...
  DataStructure dataStructure;
  auto paddedStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, DataPath({"Padded"}), {k_PaddedZ, k_PaddedY, k_PaddedX}, {1}, IDataAction::Mode::Execute);
  auto originalStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, DataPath({"Original"}), {k_DimZ, k_DimY, k_DimX}, {1}, IDataAction::Mode::Execute);
  auto outputStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, DataPath({"Output"}), {k_DimZ, k_DimY, k_DimX}, {1}, IDataAction::Mode::Execute);
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(originalStore->copyFromBuffer(0, nonstd::span<const int32>(originalValues.data(), originalValues.size())));
  SIMPLNX_RESULT_REQUIRE_VALID(paddedStore->copyFromBuffer(0, nonstd::span<const int32>(paddedValues.data(), paddedValues.size())));

  std::atomic_bool shouldCancel{false};
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::detail::CropSubtractFromStore<int32>(*paddedStore, *originalStore, *outputStore, k_Dims, k_Radius,
