...
// input mask via the shared reconstruction engine, so they share image builders, a live-ITK parity runner, and
// a parameter-setter abstraction (each filter exposes some subset of {FullyConnected, Height}).
...
inline DataPath BuildImageFromPattern(DataStructure& ds, usize dimX, usize dimY, usize dimZ, const std::vector<T>& pattern)
{
  ...
  auto store = DataStoreUtilities::CreateDataStore<T>(ds, inputPath, cellShape, {1}, IDataAction::Mode::Execute);
 ...
 *        Height only; HMinima -> both. Injecting the setter lets the one parity runner drive all three (and the
 *        matching legacy ITK filter, which shares the identical key strings).
 */
...
//------------------------------------------------------------------------------
// A single parity configuration: the op-specific params plus the image dims (the element type is iterated by the
// caller). @c height is ignored by a FullyConnected-only filter; @c fullyConnected by a Height-only filter.
struct ReconCase
{
  const char* label;
  bool fullyConnected;
 ...
 *        preflight + execute, and requires both succeed. Reuses NewFilterT::k_*_Key for BOTH the new and the
 *        legacy ITK filter -- correct only because the new filter deliberately reuses the legacy key strings
 *        ("input_image_geometry_path", "input_image_data_path", "output_array_name", "fully_connected",
 *        "height"). @p outputName is the created output array's name.
 */
template <class NewFilterT>
...
/**
 * @brief Live legacy-vs-new EXACT parity grid for a reconstruction filter of element type T. For each config,
 *        builds byte-identical input via @p buildImage into two DataStructures, runs the legacy ITK filter
 *        (created at runtime by @p legacyUuid so the target need not link ITKImageProcessing) and the new
 *        @p NewFilterT with identical Arguments, and asserts the outputs match EXACTLY (@ref CompareDataArrays).
 *        Morphological reconstruction only copies/clamps existing input values, so exact equality holds for
 *        integer AND float types -- no tolerance. Storage is forced in-core so the legacy ITK filter (which
 *        rejects out-of-core arrays) runs in any build. @tparam OutT is the OUTPUT element type used for the exact
 *        comparison; it defaults to the input type T (correct for the value-preserving filters) but is a fixed type
 *        for filters whose output type differs from the input (e.g. RegionalMaxima/Minima emit uint32 labels).
 */
template <class NewFilterT, class T, class OutT = T>
inline void RunReconstructionParityGrid(const Uuid& legacyUuid, const std::function<DataPath(DataStructure&, usize, usize, usize)>& buildImage, const std::vector<ReconCase>& configs,
                                        const ReconParamSetter& setParams)
{
  ...
  for(const ReconCase& cfg : configs)
  {
    DYNAMIC_SECTION(cfg.label)
    {
      ...
      RunReconstructionFilter<NewFilterT>(newFilter, newDs, newInput, setParams, cfg.fullyConnected, cfg.height);
      ...
      RunReconstructionFilter<NewFilterT>(*legacyFilter, legacyDs, legacyInput, setParams, cfg.fullyConnected, cfg.height);
      ...
      UnitTest::CompareDataArrays<OutT>(newOut, legacyOut);
    }
  }
}
/**
 * @brief Exact element-wise float32 comparison (NO tolerance, unlike UnitTest::CompareDataArrays which accepts a 1e-4
 *        absolute slack on any mismatch). Matched NaNs are treated as equal. Reports the first mismatch. Used for the
 *        D3 (OOC-vs-in-core) byte-match gates and for bit-exact live-ITK parity where the tolerant comparator could
 *        hide a real ULP-level divergence.
 */
 ...
 *        MinMaxCurvatureFlow, which accepts both float32 and float64) can gate BOTH widths bit-exactly against live
 *        ITK with one helper. @ref RequireExactFloat32 is kept as-is (its float32-only call sites are unaffected).
 */
template <class T>
...
//------------------------------------------------------------------------------
// ITK-sourced real-image golden helpers (coexistence-only -- the (B) live-ITK arm is dropped at ITK removal). These
// mirror the object-morphology / grayscale-morphology golden helpers: read the ITK test's real input image through
// OUR ITK-free reader, run OUR filter with the ITK case's parameters, then assert BOTH oracles --
//   (B) LIVE-ITK parity: the legacy ITK filter (resolved from the loaded plugin's replacement map) on the SAME input,
//       compared at this filter's established parity class;
//   (A) DURABLE golden: an embedded md5 of OUR output (md5-validity-first, plan Sec.4) OR a stored ITK baseline
//       compared via CompareImages at the ITK test's tolerance.
// The whole case is pinned ForceInCore: reading via OUR reader is OOC-safe, but the legacy ITK (B) run bad_casts an
// OOC store. Both filters share identical key STRINGS, so one Arguments (built from FilterT::k_*_Key) drives both;
// the OUR/ITK outputs are distinct sibling arrays in the SAME DataStructure so the input is read only once.
...
//------------------------------------------------------------------------------
// Task 9 (reconstruction + regional extrema): md5-golden. Runs OUR filter, then legacy-ITK (B, bit-exact via
// CompareImages @ tol 0), then (A) md5-validity-first vs the committed hash. @p setExtraParams sets the ITK case's
// op-specific params (Height / kernel / foreground / etc.); it defaults to none for the parameter-free filters.
...
inline void RunReconstructionMd5ItkGolden(const std::string& inputFile, const std::string& committedMd5, const ExtraParamSetter& setExtraParams = NoExtraParams())
{
  ...
  // --- run OUR filter with the ITK case's parameters ---
  Arguments args;
  ...
  Arguments itkArgs = args; // identical key strings + params drive both filters
  ...
  const Result<> parityResult = ip_golden::CompareImages(ds, geom, itkOutput, geom, output, 0.0); // integer/label output -> bit-exact
...
//------------------------------------------------------------------------------
// Task 10 (distance transforms): baseline-golden. Runs OUR filter, then legacy-ITK (B) at the filter's established
// parity class (@p bitExactB true -> RequireExactFloat32; false -> CompareDataArrays<float32> @ 1e-4, matching the
// filter's own live-parity tests), then (A) a stored ITK baseline .nrrd compared via CompareImages @ @p tolerance.
// All five distance filters emit a fixed float32 output on an integer (2th_cthead1.png) input.
template <class FilterT>
inline void RunDistanceMapItkGoldenBaseline(const std::string& inputFile, const std::string& baselineFile, float64 tolerance, bool bitExactB, const ExtraParamSetter& setExtraParams = NoExtraParams(),
                                            UnitTest::AlgorithmTestScope* scope = nullptr)
{
  ...
  // --- (B) LIVE-ITK parity: legacy ITK filter on the SAME input into the SAME ds (distinct output name). ---
  const auto legacyUuid = ip_golden::LegacyUuidFor(FilterTraits<FilterT>::uuid);
  REQUIRE(legacyUuid.has_value());
  ...
  if(bitExactB)
  {
    RequireExactFloat32(ourOut, itkOut); // integer-input distance map is bit-exact vs live ITK
  }
  else
  {
    UnitTest::CompareDataArrays<float32>(ourOut, itkOut); // the filter's established tolerant (~1e-4) parity class
  }

  // --- (A) DURABLE baseline golden: ITK-captured baseline compared at the ITK test's tolerance. ---
