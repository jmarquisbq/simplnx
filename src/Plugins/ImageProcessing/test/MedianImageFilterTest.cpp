...
TEST_CASE("ImageProcessing::BoxNeighborhoodEngine: bounded 2D blocks and tiles preserve median transfers", "[ImageProcessing][MedianImageFilter][BoxNeighborhoodEngine]")
{
  ...
  for(const TransferCase& transferCase : transferCases)
  {
    DYNAMIC_SECTION(transferCase.label)
    {
      ...
      SIMPLNX_RESULT_REQUIRE_VALID(
          ImageProcessing::ApplyBoxNeighborhood(inputStore, outputStore, SizeVec3{k_MedianTestDimX, k_MedianTestDimY, 1}, k_MedianTestRadius, median, shouldCancel, {}, targetBytes));
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(ImageProcessing::detail::CreateBoxNeighborhood2DPlan(std::numeric_limits<usize>::max(), k_MedianTestDimY, k_MedianTestRadius[0], k_MedianTestRadius[1], sizeof(int32),
