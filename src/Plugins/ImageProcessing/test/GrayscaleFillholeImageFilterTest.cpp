...
namespace
{
// Legacy ITKGrayscaleFillholeImageFilter, created at runtime by UUID (so this target does not link ITKImageProcessing).
const Uuid k_LegacyGrayscaleFillholeUuid = *Uuid::FromString("1f1dd9e4-d361-432b-a22b-5535664ee545");

// Bright field / dark hole values used by the hole-bearing builder (exactly representable in uint8/int16/float32).
constexpr float64 k_Field = 200.0;
constexpr float64 k_Hole = 40.0;

// Live-parity configs: 3D + genuinely-2D (Z=1) hole-bearing images, each with FullyConnected off and on.
...
// -----------------------------------------------------------------------------
// (1) Live-ITK EXACT parity grid across uint8 / int16 / float32, FullyConnected {off, on}, on a hole-bearing
//     image (interior dark blobs that get filled + a border-touching dark region that does not). Fillhole is a
//     reconstruction-by-erosion that only copies/clamps existing input values, so the new filter must reproduce
//     the legacy ITK Grayscale Fillhole output EXACTLY -- for integer AND float types (no tolerance).
// -----------------------------------------------------------------------------
TEMPLATE_TEST_CASE("ImageProcessing::GrayscaleFillholeImageFilter: Live-ITK exact parity grid", "[ImageProcessing][GrayscaleFillholeImageFilter]", uint8, int16, float32)
{
  using T = TestType;
  const auto build = [](DataStructure& ds, usize dx, usize dy, usize dz) { return rt::BuildHoleImage<T>(ds, dx, dy, dz, static_cast<T>(k_Field), static_cast<T>(k_Hole)); };
  rt::RunReconstructionParityGrid<GrayscaleFillholeImageFilter, T>(k_LegacyGrayscaleFillholeUuid, build, k_ParityConfigs, rt::FullyConnectedParamSetter<GrayscaleFillholeImageFilter>());
}
...
TEST_CASE("ImageProcessing::GrayscaleFillholeImageFilter: Preflight scalar guard", "[ImageProcessing][GrayscaleFillholeImageFilter]")
{
  ...
  SECTION("multi-component input is rejected")
  {
    ...
    auto vecStore = DataStoreUtilities::CreateDataStore<uint8>(ds, vecPath, {8, 8, 8}, {2}, IDataAction::Mode::Execute);
