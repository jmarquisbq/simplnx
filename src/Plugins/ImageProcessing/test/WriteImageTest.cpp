...
const std::string k_ImageStackDir = (fs::path(unit_test::k_SourceDir.view()).parent_path() / "ITKImageProcessing/data/ImageStack").string();
                   ...
                   uint64, float32, float64)
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(writer.preflight(dataStructure, writeArgs).outputActions);
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(reader.preflight(readDataStructure, readArgs).outputActions);
  SIMPLNX_RESULT_REQUIRE_VALID(reader.execute(readDataStructure, readArgs).result);
