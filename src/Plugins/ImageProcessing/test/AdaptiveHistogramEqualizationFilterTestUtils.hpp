...
//------------------------------------------------------------------------------
// A single (alpha, beta, radius, image-dims) parity configuration.
struct AheCase
{
  const char* label;
  float32 alpha;
 ...
 *        execute, and requires both succeed. Reuses NewFilterT::k_*_Key for BOTH the new and the legacy ITK
 *        filter -- correct only because the new filter deliberately reuses the legacy key strings ("radius",
 *        "alpha", "beta", "input_image_geometry_path", "input_image_data_path", "output_array_name").
 */
...
/**
 * @brief Tolerant legacy-vs-new parity grid for an element type T. For each @p configs entry, builds
 *        byte-identical input into two DataStructures, runs the legacy ITK filter (created at runtime by
 *        @p legacyUuid so the target need not link ITKImageProcessing) and the new @p NewFilterT with
 *        identical Arguments, reads both outputs into flat vectors, and compares them with @ref RequireAheClose
 *        (integer: |a-b|<=1 on <= @p maxDiffFraction of voxels; float: atol/rtol). Storage is forced in-core
 *        so the legacy ITK filter (which rejects OOC arrays) runs in any build.
 */
template <class NewFilterT, class T>
inline void RunAheParityGrid(const Uuid& legacyUuid, const std::vector<AheCase>& configs, double atol, double rtol, double maxDiffFraction)
{
  UnitTest::LoadPlugins();
  const UnitTest::PreferencesSentinel prefsSentinel(nx::core::DataStorageMode::ForceInCore, 0);
  REQUIRE(Application::Instance()->getFilterList()->createFilter(legacyUuid) != nullptr);

  for(const AheCase& cfg : configs)
  {
    DYNAMIC_SECTION(cfg.label)
    {
      DataStructure newDs;
      const DataPath newInput = BuildAheImage<T>(newDs, cfg.dimX, cfg.dimY, cfg.dimZ);
      NewFilterT newFilter;
      RunAheFilter<NewFilterT>(newFilter, newDs, newInput, cfg.alpha, cfg.beta, cfg.radius);
      ...
      const DataPath legacyInput = BuildAheImage<T>(legacyDs, cfg.dimX, cfg.dimY, cfg.dimZ);
      RunAheFilter<NewFilterT>(*legacyFilter, legacyDs, legacyInput, cfg.alpha, cfg.beta, cfg.radius);

      const DataPath outputPath({"Image Geometry", "CellData", "Output"});
      const auto& newStore = newDs.getDataRefAs<DataArray<T>>(outputPath).getDataStoreRef();
      const auto& legacyStore = legacyDs.getDataRefAs<DataArray<T>>(outputPath).getDataStoreRef();
      REQUIRE(newStore.getSize() == legacyStore.getSize());
      std::vector<T> newOut(newStore.getSize());
      std::vector<T> legacyOut(legacyStore.getSize());
      for(usize i = 0; i < newOut.size(); ++i)
      {
        newOut[i] = newStore.getValue(i);
        legacyOut[i] = legacyStore.getValue(i);
      }
      RequireAheClose<T>(newOut, legacyOut, atol, rtol, maxDiffFraction);
    }
  }
}
} // namespace ahe_test
