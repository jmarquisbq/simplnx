...
TEST_CASE("ImageProcessing::BinaryThresholdImageFilter: Legacy parity", "[ImageProcessing][BinaryThresholdImageFilter]")
{
  ip_test::RunLegacyParity<BinaryThresholdImageFilter, float32>(
      *Uuid::FromString("ba2494b0-c4f0-43ff-9d08-900395900e0c"),
      [](Arguments& args) {
        args.insertOrAssign(BinaryThresholdImageFilter::k_LowerThreshold_Key, std::make_any<float64>(100.0));
        args.insertOrAssign(BinaryThresholdImageFilter::k_UpperThreshold_Key, std::make_any<float64>(500.0));
...
// with the ITK case's params, then (B) live-ITK parity FIRST (bit-exact uint8) and (A) md5-validity-first (plan
// Sec.4). Pinned ForceInCore: the legacy ITK filter bad_casts an OOC store (reading via OUR reader is OOC-safe).
// -----------------------------------------------------------------------------
void RunBinaryThresholdItkGolden(const std::function<void(Arguments&)>& setParams, const std::string& committedMd5)
{
  ...
  const auto legacyUuid = ip_golden::LegacyUuidFor(FilterTraits<BinaryThresholdImageFilter>::uuid);
  REQUIRE(legacyUuid.has_value());
  Arguments itkArgs = args;
  itkArgs.insertOrAssign(BinaryThresholdImageFilter::k_OutputImageArrayName_Key, std::make_any<std::string>("ITK Output"));
...
TEST_CASE("ImageProcessing::BinaryThresholdImageFilter: Preflight rejects Lower > Upper", "[ImageProcessing][BinaryThresholdImageFilter]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(filter.preflight(ds, makeArgs(250.0, 250.0)).outputActions);
  SIMPLNX_RESULT_REQUIRE_VALID(filter.preflight(ds, makeArgs(100.0, 500.0)).outputActions);
}
