...
TEST_CASE("ImageProcessing::ReadImageFilter: NRRD live-ITK read parity", "[ImageProcessing][ReadImageFilter][itk-parity]")
{
  UnitTest::LoadPlugins();

  // The legacy ITK reader (oracle) adopts its buffer into the store via a dynamic_cast to the
  // in-core DataStore<T>, so it cannot write an out-of-core store (throws bad_cast). Read-value
  // parity is storage-independent, so pin BOTH readers to in-core here; our reader's OOC path is
  // covered separately by the OOC byte-match test. (Same reason every legacy-ITK-filter test does this.)
  ...
  {
    WARN("ITKImageProcessing plugin not loaded (ITK reader UUID d72eaf98... unavailable); skipping live-ITK NRRD parity gate.");
    return;
  }

  const fs::path dir = ItkJsonFiltersDir();
  // Axis-aligned inputs/baselines: the clean parity set (scalar + RGB/vector, raw + gzip, 2D + 3D).
  const std::vector<fs::path> files = {
      dir / "Input" / "RA-Short.nrrd",                                            // short, gzip, 3D
      dir / "Input" / "RA-Float.nrrd",                                            // float, gzip, 3D
      dir / "Input" / "RA-Slice-Short.nrrd",                                      // short, gzip, 3D w/ Z=1
      dir / "Input" / "DSA.nrrd",                                                 // uchar, raw, 2D
      dir / "Input" / "VM1111Shrink-RGBNorm.nrrd",                                // float vector-3, gzip
      dir / "Baseline" / "BasicFilters_AbsImageFilter_short.nrrd",                // short, gzip, 2D-as-3D
      dir / "Baseline" / "BasicFilters_AdditiveGaussianNoiseImageFilter_rgb.nrrd" // uchar vector-3, raw
  };
  ...
  for(const auto& file : files)
  {
    DYNAMIC_SECTION("file=" << file.filename().string())
    {
      ...
      // Ours. Bind results to locals: SIMPLNX_RESULT_REQUIRE_VALID evaluates its
      // argument multiple times, and preflight()/execute() apply their
      // create-actions to `oursDs`, so passing the calls inline would re-run them.
      DataStructure oursDs;
      ReadImageFilter ours;
      const DataPath oursGeom({"OursGeom"});
      const DataPath oursArr = oursGeom.createChildPath("Cell Data").createChildPath("ImageData");
      const Arguments oursArgs = MakeReadImageNrrdArgs(file, oursGeom, "Cell Data", "ImageData");
...
// Empirically pins the endian-required behavior (fix A2) against live ITK: a
// multi-byte type with a raw/gzip encoding and NO 'endian' field must be REJECTED
// by both readers. teem/ITK's NrrdIO errors with "type (...) and encoding (...)
// require endian info"; our parser returns -35713. Self-skips (WARN) when the ITK
// plugin is not loaded so ITK-free ImageProcessing builds still pass.
TEST_CASE("ImageProcessing::ReadImageFilter: NRRD missing-endian rejection matches ITK", "[ImageProcessing][ReadImageFilter][itk-parity]")
{
  UnitTest::LoadPlugins();

  // The legacy ITK reader adopts its buffer into the store via a dynamic_cast to the
  // in-core DataStore<T>, so it throws bad_cast on an out-of-core store. Pin storage
  // in-core for the ITK oracle, exactly like every other [itk-parity] test here.
  const UnitTest::PreferencesSentinel prefsSentinel(nx::core::DataStorageMode::ForceInCore, 0);

  // int16 payload, little-endian bytes, but the header omits 'endian' entirely.
  std::vector<int16_t> voxels(2 * 2 * 2);
  std::iota(voxels.begin(), voxels.end(), static_cast<int16_t>(0));
  const fs::path p = ReadImageOutputDir() / "no_endian_int16.nrrd";
  WriteNrrdRaw(p, {"NRRD0004", "type: short", "dimension: 3", "sizes: 2 2 2", "kinds: domain domain domain", "encoding: raw"}, ToBytes(voxels));

  // Ours: preflight must fail with the dedicated -35713 error.
  {
    DataStructure ds;
    ReadImageFilter filter;
    const Arguments args = MakeReadImageNrrdArgs(p, DataPath({"NRRD NoEndian"}), "Cell Data", "ImageData");
    const auto preflight = filter.preflight(ds, args);
    REQUIRE(preflight.outputActions.invalid());
    REQUIRE(preflight.outputActions.errors()[0].code == -35713);
  }

  // ITK oracle: the same file must also be rejected by ITK's NrrdIO.
  auto app = Application::GetOrCreateInstance();
  const Uuid itkReaderUuid = *Uuid::FromString("d72eaf98-9b1d-44c9-88f2-a5c3cf57b4f2");
  auto itkReader = app->getFilterList()->createFilter(itkReaderUuid);
  if(itkReader == nullptr)
  {
    WARN("ITKImageProcessing plugin not loaded (ITK reader UUID d72eaf98... unavailable); skipping the live-ITK missing-endian parity check.");
    return;
  }
  DataStructure itkDs;
  Arguments itkArgs;
  itkArgs.insertOrAssign(k_ITK_FileName, std::make_any<fs::path>(p));
  itkArgs.insertOrAssign(k_ITK_GeometryPath, std::make_any<DataPath>(DataPath({"ItkGeom"})));
  itkArgs.insertOrAssign(k_ITK_CellDataName, std::make_any<std::string>("Cell Data"));
  itkArgs.insertOrAssign(k_ITK_ArrayName, std::make_any<std::string>("ImageData"));
  itkArgs.insertOrAssign(k_ITK_ChangeOrigin, std::make_any<bool>(false));
  itkArgs.insertOrAssign(k_ITK_ChangeSpacing, std::make_any<bool>(false));
  const auto itkExec = itkReader->execute(itkDs, itkArgs); // execute() runs preflight internally
  REQUIRE(itkExec.result.invalid());
}

// Empirically pins the named-'space' -> LPS origin conversion (fix H2) against live ITK: a
// synthetic RAS NRRD's origin (10,20,30) must read back as (-10,-20,30) -- X and Y negated,
// Z unchanged -- from BOTH our reader and ITK's NrrdIO. Self-skips (WARN) when the ITK
// plugin is not loaded so ITK-free ImageProcessing builds still pass.
TEST_CASE("ImageProcessing::ReadImageFilter: NRRD RAS origin parity matches ITK", "[ImageProcessing][ReadImageFilter][itk-parity]")
{
  UnitTest::LoadPlugins();

  // Same reason as the parity gate: the ITK oracle can only write an in-core store.
  const UnitTest::PreferencesSentinel prefsSentinel(nx::core::DataStorageMode::ForceInCore, 0);

  std::vector<int16_t> voxels(2 * 2 * 2);
  std::iota(voxels.begin(), voxels.end(), static_cast<int16_t>(0));
  const fs::path p = ReadImageOutputDir() / "ras_origin_parity.nrrd";
  WriteNrrdRaw(p,
               {"NRRD0004", "type: short", "dimension: 3", "space: right-anterior-superior", "sizes: 2 2 2", "space directions: (1,0,0) (0,1,0) (0,0,1)", "kinds: domain domain domain",
                "endian: little", "encoding: raw", "space origin: (10,20,30)"},
               ToBytes(voxels));
  ...
  DataStructure oursDs;
  ReadImageFilter ours;
  const DataPath oursGeom({"OursRAS"});
  const Arguments oursArgs = MakeReadImageNrrdArgs(p, oursGeom, "Cell Data", "ImageData");
  const auto oursPreflight = ours.preflight(oursDs, oursArgs);
  SIMPLNX_RESULT_REQUIRE_VALID(oursPreflight.outputActions);
  const auto oursExecute = ours.execute(oursDs, oursArgs);
  SIMPLNX_RESULT_REQUIRE_VALID(oursExecute.result);
  const auto& og = oursDs.getDataRefAs<ImageGeom>(oursGeom);
  REQUIRE(og.getOrigin()[0] == Approx(-10.0f));
  REQUIRE(og.getOrigin()[1] == Approx(-20.0f));
  REQUIRE(og.getOrigin()[2] == Approx(30.0f));
  ...
  if(itkReader == nullptr)
  {
    WARN("ITKImageProcessing plugin not loaded (ITK reader UUID d72eaf98... unavailable); skipping live-ITK RAS origin parity.");
    return;
  }
  DataStructure itkDs;
  const DataPath itkGeom({"ItkRAS"});
  Arguments itkArgs;
  itkArgs.insertOrAssign(k_ITK_FileName, std::make_any<fs::path>(p));
  itkArgs.insertOrAssign(k_ITK_GeometryPath, std::make_any<DataPath>(itkGeom));
  itkArgs.insertOrAssign(k_ITK_CellDataName, std::make_any<std::string>("Cell Data"));
...
TEST_CASE("ImageProcessing::ReadImageFilter: 3D TIFF live-ITK read parity", "[ImageProcessing][ReadImageFilter][itk-parity]")
{
  UnitTest::LoadPlugins();

  // The legacy ITK reader adopts its buffer into the in-core DataStore<T> via a dynamic_cast, so it
  // cannot write an out-of-core store (throws bad_cast). Read-value parity is storage-independent, so
  // pin BOTH readers in-core here; the raster OOC path is covered by the D3 byte-match case below.
  // (Same reason every legacy-ITK-filter parity test in this file does this.)
  const UnitTest::PreferencesSentinel prefsSentinel(nx::core::DataStorageMode::ForceInCore, 0);
  ...
  if(itkReader == nullptr)
  {
    WARN("ITKImageProcessing plugin not loaded (ITK reader UUID d72eaf98... unavailable); skipping live-ITK 3D-TIFF parity gate.");
    return;
  }
  ...
  // Reads `tiffPath` with ReadImageFilter and the ITK oracle, then asserts (a) BOTH produce a Z==3
  // geometry (confirming ITK reads the multi-page TIFF as 3D, so the parity oracle is valid),
  // (b) identical dims/spacing/origin, and (c) byte-identical cell arrays.
  auto runParity = [&](const fs::path& tiffPath) {
    // Ours. Bind results to locals: SIMPLNX_RESULT_REQUIRE_VALID evaluates its argument multiple
    // times, and preflight()/execute() apply their create-actions to `oursDs`.
    DataStructure oursDs;
    ReadImageFilter ours;
    const DataPath oursGeom({"OursGeom"});
    const DataPath oursArr = oursGeom.createChildPath("Cell Data").createChildPath("ImageData");
    const Arguments oursArgs = MakeReadImageNrrdArgs(tiffPath, oursGeom, "Cell Data", "ImageData");
    const auto oursPreflight = ours.preflight(oursDs, oursArgs);
    SIMPLNX_RESULT_REQUIRE_VALID(oursPreflight.outputActions);
    const auto oursExecute = ours.execute(oursDs, oursArgs);
    SIMPLNX_RESULT_REQUIRE_VALID(oursExecute.result);
    ...
    const DataPath itkArr = itkGeom.createChildPath("Cell Data").createChildPath("ImageData");
    Arguments itkArgs;
    itkArgs.insertOrAssign(k_ITK_FileName, std::make_any<fs::path>(tiffPath));
    itkArgs.insertOrAssign(k_ITK_GeometryPath, std::make_any<DataPath>(itkGeom));
    itkArgs.insertOrAssign(k_ITK_CellDataName, std::make_any<std::string>("Cell Data"));
    itkArgs.insertOrAssign(k_ITK_ArrayName, std::make_any<std::string>("ImageData"));
    itkArgs.insertOrAssign(k_ITK_ChangeOrigin, std::make_any<bool>(false));
    ...
    // (a) Both read the multi-page TIFF as a 3D Z==3 volume -- confirms the ITK oracle is valid for 3D TIFF.
    const auto& og = oursDs.getDataRefAs<ImageGeom>(oursGeom);
    const auto& ig = itkDs.getDataRefAs<ImageGeom>(itkGeom);
    REQUIRE(og.getDimensions()[2] == numPages);
    REQUIRE(ig.getDimensions()[2] == numPages);

    // (b) Identical geometry dims / spacing / origin.
    REQUIRE(og.getDimensions() == ig.getDimensions());
    for(usize k = 0; k < 3; k++)
    {
      REQUIRE(og.getOrigin()[k] == Approx(ig.getOrigin()[k]).margin(1e-4));
    ...
    // (c) Byte-identical cell arrays.
    RequireArraysIdentical(oursDs.getDataRefAs<IDataArray>(oursArr), itkDs.getDataRefAs<IDataArray>(itkArr));
  };

  SECTION("uint8 3-page")
  {
    // pixel(page p, index i) = p*50 + i + 1, distinct per page and per in-plane position (all < 256).
    std::vector<std::vector<uint8_t>> pages(numPages, std::vector<uint8_t>(static_cast<size_t>(width) * height));
    for(usize p = 0; p < numPages; ++p)
    {
      for(size_t i = 0; i < pages[p].size(); ++i)
      {
        pages[p][i] = static_cast<uint8_t>(p * 50 + i + 1);
      }
    }
    const fs::path tiffPath = ReadImageOutputDir() / "parity_3page_uint8.tif";
    WriteMultiPageTiff(tiffPath, width, height, pages);
    runParity(tiffPath);
  }

  SECTION("uint16 3-page")
  {
    // Values above 255 so the uint16 path is genuinely exercised (would truncate if read as uint8).
    std::vector<std::vector<uint16_t>> pages(numPages, std::vector<uint16_t>(static_cast<size_t>(width) * height));
    for(usize p = 0; p < numPages; ++p)
    {
      for(size_t i = 0; i < pages[p].size(); ++i)
      {
        pages[p][i] = static_cast<uint16_t>(p * 1000 + i * 7 + 300);
      }
    }
    const fs::path tiffPath = ReadImageOutputDir() / "parity_3page_uint16.tif";
    WriteMultiPageTiffTyped<uint16_t>(tiffPath, width, height, 1, pages);
    runParity(tiffPath);
  }
}
