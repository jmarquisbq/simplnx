...
TEST_CASE("ImageProcessing::FiniteDifferenceEngine: real-OOC selector uses resident state only after a complete grant", "[ImageProcessing][FiniteDifferenceEngine][WorkingMemory]")
{
  ...
  auto run = [&](uint64 budgetBytes) {
    ...
    SIMPLNX_RESULT_REQUIRE_VALID(ApplyFiniteDifference(inputStore, outputStore, dims, FloatVec3{1.0f, 1.0f, 1.0f}, function, 0.05, 2, shouldCancel, messageHandler));
...
TEST_CASE("ImageProcessing::FiniteDifferenceEngine: final 3D iteration writes output without a scratch round trip", "[ImageProcessing][FiniteDifferenceEngine]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(inputStore.copyFromBuffer(0, field));
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(outputStore.copyIntoBuffer(0, actual));
...
TEST_CASE("ImageProcessing::FiniteDifferenceEngine: direct interior neighbors remain bit-exact with clamped edges", "[ImageProcessing][FiniteDifferenceEngine]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(residentInput.copyFromBuffer(0, nonstd::span<const float32>(values.data(), values.size())));
  std::atomic_bool shouldCancel{false};
  IFilter::MessageHandler messageHandler{};
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(oocInput.copyFromBuffer(0, nonstd::span<const float32>(values.data(), values.size())));
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(residentOutput.copyIntoBuffer(0, nonstd::span<float32>(expected.data(), expected.size())));
  SIMPLNX_RESULT_REQUIRE_VALID(oocOutput.copyIntoBuffer(0, nonstd::span<float32>(actual.data(), actual.size())));
  REQUIRE(actual == expected);
}
