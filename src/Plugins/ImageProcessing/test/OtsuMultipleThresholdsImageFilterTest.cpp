...
namespace
{
// Legacy ITK Otsu filter, created at runtime by UUID so the ImageProcessing test target does not
// link against the ITKImageProcessing plugin.
const Uuid k_LegacyOtsuUuid = *Uuid::FromString("30f37bcd-701f-4e64-aa9d-1181469d3fb5");
 ...
 *        so the two DataStructures built for a legacy-vs-new parity run get byte-identical input.
 */
DataPath BuildMultimodalImage(DataStructure& ds, usize dim)
{
  ...
  auto store = DataStoreUtilities::CreateDataStore<float32>(ds, inputPath, cellShape, {1}, IDataAction::Mode::Execute);
...
DataPath BuildCompactInt16Image(DataStructure& ds, usize dim)
{
  ...
  const DataPath inputPath({"Image Geometry", "CellData", "Input"});
  auto store = DataStoreUtilities::CreateDataStore<int16>(ds, inputPath, cellShape, {1}, IDataAction::Mode::Execute);
...
// -----------------------------------------------------------------------------
// (1) Legacy parity grid: the new filter must produce the EXACT (integer) uint8 label image that the
//     legacy ITKOtsuMultipleThresholdsImageFilter produces, over a grid of every relevant option. This
//     is the correctness gate for the HistogramEngine backend + classify convention.
// -----------------------------------------------------------------------------
TEST_CASE("ImageProcessing::OtsuMultipleThresholdsImageFilter: Legacy parity grid (multimodal 3D)", "[ImageProcessing][OtsuMultipleThresholdsImageFilter]")
{
  UnitTest::LoadPlugins();
  const UnitTest::PreferencesSentinel prefsSentinel(nx::core::DataStorageMode::ForceInCore, 0);

  const uint8 numThresholds = GENERATE(uint8{1}, uint8{2}, uint8{3});
  const uint32 numBins = GENERATE(uint32{128}, uint32{64});
  const bool valleyEmphasis = GENERATE(false, true);
  const bool returnBinMidpoint = GENERATE(false, true);
  CAPTURE(static_cast<uint32>(numThresholds), numBins, valleyEmphasis, returnBinMidpoint);
  ...
  auto run = [&](IFilter& filter, DataStructure& ds, const DataPath& inputPath) {
    ...
    args.insertOrAssign(OtsuMultipleThresholdsImageFilter::k_ValleyEmphasis_Key, std::make_any<bool>(valleyEmphasis));
    args.insertOrAssign(OtsuMultipleThresholdsImageFilter::k_ReturnBinMidpoint_Key, std::make_any<bool>(returnBinMidpoint));
    auto preflightResult = filter.preflight(ds, args);
    SIMPLNX_RESULT_REQUIRE_VALID(preflightResult.outputActions);
    auto executeResult = filter.execute(ds, args);
    SIMPLNX_RESULT_REQUIRE_VALID(executeResult.result);
  ...
  DataStructure newDs;
  const DataPath newInput = BuildMultimodalImage(newDs, 16);
  OtsuMultipleThresholdsImageFilter newFilter;
  run(newFilter, newDs, newInput);

  IFilter::UniquePointer legacyFilter = Application::Instance()->getFilterList()->createFilter(k_LegacyOtsuUuid);
  REQUIRE(legacyFilter != nullptr);
  DataStructure legacyDs;
  const DataPath legacyInput = BuildMultimodalImage(legacyDs, 16);
  ...
  REQUIRE(legacyOut.getDataType() == DataType::uint8);
  REQUIRE(newOut.getDataType() == DataType::uint8);
  // EXACT integer-label match, voxel for voxel.
  UnitTest::CompareDataArrays<uint8>(legacyOut, newOut);
}
...
// output) with the ITK case's params, then (B) live-ITK parity FIRST (bit-exact uint8 labels) and (A) md5-validity-first
// (plan Sec.4). Pinned ForceInCore: the legacy ITK filter bad_casts an OOC store (reading via OUR reader is OOC-safe).
// -----------------------------------------------------------------------------
void RunOtsuItkGolden(const std::string& inputFile, const std::function<void(Arguments&)>& setParams, const std::string& committedMd5)
{
  ...
  // (B) LIVE-ITK parity: legacy ITK filter on the SAME input into the SAME ds, bit-exact (integer labels).
  const auto legacyUuid = ip_golden::LegacyUuidFor(FilterTraits<OtsuMultipleThresholdsImageFilter>::uuid);
  REQUIRE(legacyUuid.has_value());
  Arguments itkArgs = args;
  itkArgs.insertOrAssign(OtsuMultipleThresholdsImageFilter::k_OutputImageArrayName_Key, std::make_any<std::string>("ITK Output"));
