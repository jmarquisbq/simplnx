...
TEST_CASE("ImageProcessing::ApplyPointwise doubles every value", "[ImageProcessing]")
{
  ...
  auto inStore = DataStoreUtilities::CreateDataStore<int32>(ds, inputPath, tupleShape, {1}, IDataAction::Mode::Execute);
  auto* inArray = DataArray<int32>::Create(ds, "input", inStore, {});
  auto outStore = DataStoreUtilities::CreateDataStore<int32>(ds, outputPath, tupleShape, {1}, IDataAction::Mode::Execute);
...
TEST_CASE("ImageProcessing::ApplyPointwise different-type buffered transfer retains two-buffer batches", "[ImageProcessing][ApplyPointwise]")
{
  ...
  std::atomic_bool shouldCancel{false};
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::detail::ApplyPointwiseImpl(inputStore, outputStore, cast, shouldCancel, {}, options));
...
TEST_CASE("ImageProcessing::ApplyPointwise same-type buffered execution supports store aliasing", "[ImageProcessing][ApplyPointwise]")
{
  ...
  const auto doubler = [](int32 value) -> int32 { return value * 2; };
  std::atomic_bool shouldCancel{false};
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::detail::ApplyPointwiseImpl(store, store, doubler, shouldCancel, {}, {2 * sizeof(int32), true}));
...
TEST_CASE("ImageProcessing::ApplyPointwise buffered target smaller than one value uses one-value batches", "[ImageProcessing][ApplyPointwise]")
{
  ...
  std::atomic_bool shouldCancel{false};
  SIMPLNX_RESULT_REQUIRE_VALID(ImageProcessing::detail::ApplyPointwiseImpl(inputStore, outputStore, increment, shouldCancel, {}, {1, true}));
...
TEST_CASE("ImageProcessing::ApplyPointwise preserves bytes on non-power-of-two OOC stores", "[ImageProcessing][ApplyPointwise][OOC]")
{
  ...
  const DataPath inputPath({"Input"});
  const DataPath outputPath({"Output"});
  auto inputStore = DataStoreUtilities::CreateDataStore<uint8>(dataStructure, inputPath, tupleShape, {1}, IDataAction::Mode::Execute);
  auto outputStore = DataStoreUtilities::CreateDataStore<uint8>(dataStructure, outputPath, tupleShape, {1}, IDataAction::Mode::Execute);
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(inputArray->getDataStoreRef().copyFromBuffer(0, nonstd::span<const uint8>(input.data(), input.size())));
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(outputArray->getDataStoreRef().copyIntoBuffer(0, nonstd::span<uint8>(actual.data(), actual.size())));
