                             ...
                             usize maximumWriteValues, usize expectedWriteCount)
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(ApplyAdaptiveHistogramEqualization(inputStore, outputStore, SizeVec3{dimX, dimY, 1}, radius, alpha, beta, shouldCancel, messageHandler, targetBytes));

  std::vector<T> output(input.size());
  SIMPLNX_RESULT_REQUIRE_VALID(outputStore.copyIntoBuffer(0, nonstd::span<T>(output.data(), output.size())));
          ...
          "[ImageProcessing][AdaptiveHistogramEqualizationEngine][WorkingMemory]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(
          ...
          "[ImageProcessing][AdaptiveHistogramEqualizationEngine][WorkingMemory]")
{
  ...
  auto run = [&](uint64 budgetBytes) {
    ...
    std::atomic_bool shouldCancel{false};
    IFilter::MessageHandler messageHandler{};
    SIMPLNX_RESULT_REQUIRE_VALID(ApplyAdaptiveHistogramEqualization(inputStore, outputStore, dims, radius, 1.0f, 0.25f, shouldCancel, messageHandler));
...
TEST_CASE("ImageProcessing::AdaptiveHistogramEqualizationEngine: bounded 2D fast and generic transfers", "[ImageProcessing][AdaptiveHistogramEqualizationEngine]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(ImageProcessing::detail::CreateAdaptiveHistogramEqualization2DPlan(dimX, dimY, radius[0], radius[1], sizeof(uint8), /*targetBytes=*/1, /*fixedScratchBytes=*/0,
                                                                                                    /*useHorizontalSums=*/true));
  SIMPLNX_RESULT_REQUIRE_INVALID(ImageProcessing::detail::CreateAdaptiveHistogramEqualization2DPlan(std::numeric_limits<usize>::max(), dimY, radius[0], radius[1], sizeof(uint8),
...
TEST_CASE("ImageProcessing::AdaptiveHistogramEqualizationEngine: bounded 2D constant and cancellation preserve output", "[ImageProcessing][AdaptiveHistogramEqualizationEngine]")
{
  ...
  SECTION("cancel after transform read performs no output write")
  {
    ...
    const usize span = static_cast<usize>(*maximum - *minimum);
    const usize lutBytes = (2 * span + 1) * sizeof(float);
    SIMPLNX_RESULT_REQUIRE_VALID(ApplyAdaptiveHistogramEqualization(cancellingInputStore, outputStore, SizeVec3{dimX, dimY, 1}, radius, 1.0f, 0.25f, shouldCancel, messageHandler,
