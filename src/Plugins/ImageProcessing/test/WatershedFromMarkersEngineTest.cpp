                                     ...
                                     const std::shared_ptr<TemporaryRecordStoreStats>& voxelStats = {}, usize targetBytes = ImageProcessing::detail::k_WatershedExternalTargetBytes)
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(outputStore.copyIntoBuffer(0, nonstd::span<uint32>(output.data(), output.size())));
  return output;
}
} // namespace
                   ...
                   int32, uint64, int64, float32, float64)
{
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(ImageProcessing::detail::CreateWatershedExternalMemoryPlan<T>(0));
}
...
TEMPLATE_TEST_CASE("ImageProcessing::WatershedExternalMemory: larger working-memory targets retain more fixed-size pages", "[ImageProcessing][WatershedFromMarkersEngine]", uint8, float32)
{
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(ImageProcessing::detail::CreateWatershedExternalMemoryPlan<T>(k_ValueCount, /*targetBytes=*/1));
}
...
TEST_CASE("ImageProcessing::WatershedExternalMemory: tiled voxel layout is a checked 64-bit bijection", "[ImageProcessing][WatershedFromMarkersEngine]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(ImageProcessing::detail::CreateWatershedTiledRecordLayout(k_Dims, 0));
}
          ...
          "[ImageProcessing][WatershedFromMarkersEngine][WorkingMemory]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(ImageProcessing::detail::CalculateWatershedResidentWorkingMemoryBytes<float32>(k_ValueCount, /*markWatershedLine=*/true));
...
TEST_CASE("ImageProcessing::WatershedExternalMemory: external min-heap preserves level and FIFO order", "[ImageProcessing][WatershedFromMarkersEngine]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(heap.push(5, 50));
  SIMPLNX_RESULT_REQUIRE_VALID(heap.push(1, 10));
  SIMPLNX_RESULT_REQUIRE_VALID(heap.push(5, 51));
  SIMPLNX_RESULT_REQUIRE_VALID(heap.push(-2, 20));
  SIMPLNX_RESULT_REQUIRE_VALID(heap.push(ImageProcessing::detail::WatershedEffectiveLevel<int32>(3, 5), 52));
  SIMPLNX_RESULT_REQUIRE_VALID(heap.push(ImageProcessing::detail::WatershedEffectiveLevel<int32>(8, 5), 80));
  ...
  REQUIRE(heap.empty());
  SIMPLNX_RESULT_REQUIRE_INVALID(heap.pop());
  SIMPLNX_RESULT_REQUIRE_VALID(heap.flush());

  shouldCancel = true;
  SIMPLNX_RESULT_REQUIRE_INVALID(heap.push(1, 1));
}
...
TEST_CASE("ImageProcessing::WatershedExternalMemory: external min-heap rejects capacity and provider metadata", "[ImageProcessing][WatershedFromMarkersEngine]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(heapResult);
  SIMPLNX_RESULT_REQUIRE_VALID(heapResult.value()->push(1.5f, 1));
  SIMPLNX_RESULT_REQUIRE_VALID(heapResult.value()->push(1.5f, 2));
  SIMPLNX_RESULT_REQUIRE_INVALID(heapResult.value()->push(1.5f, 3));
  ...
  SIMPLNX_RESULT_REQUIRE_INVALID(Heap::Create(std::move(wrongStore.value()), /*recordsPerPage=*/2, /*maximumPages=*/1, shouldCancel));
}
...
TEST_CASE("ImageProcessing::WatershedExternalMemory: fixed-level external buckets preserve numeric and FIFO order", "[ImageProcessing][WatershedFromMarkersEngine]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(queue.push(5, 0));
  SIMPLNX_RESULT_REQUIRE_VALID(queue.push(-2, 1));
  SIMPLNX_RESULT_REQUIRE_VALID(queue.push(5, 2));
  SIMPLNX_RESULT_REQUIRE_VALID(queue.push(std::numeric_limits<int16>::lowest(), 3));
  SIMPLNX_RESULT_REQUIRE_VALID(queue.push(std::numeric_limits<int16>::max(), 4));
  const int64 largeFlatIndex = static_cast<int64>(std::numeric_limits<uint32>::max()) + 7;
  SIMPLNX_RESULT_REQUIRE_VALID(queue.push(ImageProcessing::detail::WatershedEffectiveLevel<int16>(3, 5), largeFlatIndex));
  ...
  REQUIRE(queue.empty());
  SIMPLNX_RESULT_REQUIRE_INVALID(queue.pop());

  SIMPLNX_RESULT_REQUIRE_VALID(queue.push(7, 6));
  SIMPLNX_RESULT_REQUIRE_VALID(queue.push(7, 7));
  auto firstInterleaved = queue.pop();
  SIMPLNX_RESULT_REQUIRE_VALID(firstInterleaved);
  REQUIRE(firstInterleaved.value().flatIndex == 6);
  SIMPLNX_RESULT_REQUIRE_VALID(queue.push(7, largeFlatIndex));
  ...
  REQUIRE(queue.empty());
  SIMPLNX_RESULT_REQUIRE_VALID(queue.flush());
}
