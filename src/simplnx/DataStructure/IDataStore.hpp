...
public:
  ...
  /**
   * @brief Returns the store type this array will have once it is materialized.
   *
   * For a real store (InMemory or OutOfCore) this is identical to
   * getStoreType(). The distinction only matters for the Empty placeholder
   * used during preflight: getStoreType() reports Empty (data is not yet
   * available), while getPlannedStoreType() reports the backing the store
   * will be replaced with when execution begins (InMemory or OutOfCore).
   *
   * This lets callers that need to reflect the eventual storage strategy of a
   * preflight-only array -- such as the data structure view choosing an
   * in-memory vs. out-of-core icon -- do so without a type-dispatch on the
   * placeholder's element type. The default implementation returns
   * getStoreType(); EmptyDataStore overrides it.
   * @return StoreType
   */
