 ...
 *
 * X-contiguous rows make destination writes sequential.
 * Consecutive destination rows that map to the same source row reuse one bulk read.
 * Working memory is proportional to source and destination X dimensions, not total cell count.
 */
template <typename T>
class ResampleImageGeomArrayImpl
{
public:
   ...
   *
   * An out-of-bounds axis position writes zeros.
   * Cancellation stops before a later Z slice and keeps completed destination rows.
   * The task stores the first bulk-I/O error in the shared result.
   */
  void operator()() const
  {
