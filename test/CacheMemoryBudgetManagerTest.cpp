...
TEST_CASE("CacheMemoryBudgetManager reservePinned uses the effective cache budget", "[CacheMemoryBudgetManager][pin][reservation]")
{
  auto& mgr = CacheMemoryBudgetManager::instance();
  mgr.clear();
  mgr.setBudgetBytes(1000);
  ...
  {
    auto workingMemory = mgr.reserveWorkingMemory(250);
    REQUIRE(workingMemory.sizeBytes() == 250);
    CHECK_FALSE(mgr.reservePinned("combined-reservation", "blocked", 751).has_value());
    CHECK(mgr.pinnedBytes() == 0);
  }

  const auto admittedHandle = mgr.reservePinned("combined-reservation", "admitted", 751);
  REQUIRE(admittedHandle.has_value());
  CHECK(mgr.pinnedBytes() == 751);
  mgr.release(*admittedHandle);
  mgr.clear();
  mgr.setBudgetBytes(CacheMemoryBudgetManager::defaultBudgetBytes());
}

TEST_CASE("CacheMemoryBudgetManager pinWithinBudget uses the effective cache budget", "[CacheMemoryBudgetManager][pin][reservation]")
{
  auto& mgr = CacheMemoryBudgetManager::instance();
  mgr.clear();
  mgr.setBudgetBytes(1000);

  CacheMemoryBudgetManager::AllocationHandle handle = 0;
  {
    auto workingMemory = mgr.reserveWorkingMemory(250);
    REQUIRE(workingMemory.sizeBytes() == 250);
    auto [candidateHandle, evictions] = mgr.allocate("combined-pin", "candidate", 800, []() {});
    REQUIRE(evictions.empty());
    handle = candidateHandle;
    CHECK(mgr.pinWithinBudget(handle) == CacheMemoryBudgetManager::PinResult::BudgetExceeded);
    CHECK(mgr.pinnedBytes() == 0);
  }

  REQUIRE(mgr.pinWithinBudget(handle) == CacheMemoryBudgetManager::PinResult::Success);
  CHECK(mgr.pinnedBytes() == 800);
  REQUIRE(mgr.pinWithinBudget(handle) == CacheMemoryBudgetManager::PinResult::Success);
  CHECK(mgr.pinnedBytes() == 800);
  REQUIRE(mgr.unpin(handle));
  REQUIRE(mgr.unpin(handle));
  mgr.release(handle);
  mgr.clear();
  mgr.setBudgetBytes(CacheMemoryBudgetManager::defaultBudgetBytes());
