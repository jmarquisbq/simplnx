...
TEST_CASE("ParentGeometrySupportsOoc: false under unstructured, true under image/none", "[Core][FormatResolver]")
{
  DataStructure ds;

  // Array path under an ImageGeom's cell AM -> OOC-eligible (path need not exist; the helper walks containers)
  ...
  // Array path under a TriangleGeom -> NOT OOC-eligible
  ...
  // Array path with no geometry ancestor -> OOC-eligible
  DataGroup::Create(ds, "Group");
  const DataPath grpArrayPath({"Group", "arr"});

  // Array path under a RectGridGeom's cell AM -> OOC-eligible
  ...
  REQUIRE(ArrayCreationUtilities::ParentGeometrySupportsOoc(ds, imgArrayPath) == true);
  REQUIRE(ArrayCreationUtilities::ParentGeometrySupportsOoc(ds, triArrayPath) == false);
  REQUIRE(ArrayCreationUtilities::ParentGeometrySupportsOoc(ds, grpArrayPath) == true);
  REQUIRE(ArrayCreationUtilities::ParentGeometrySupportsOoc(ds, rectArrayPath) == true);
}
