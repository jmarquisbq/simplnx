...
TEST_CASE("DataIOCollection forwards datastore chunk-shape hints and CreateArrayAction retains them", "[TemporaryRecordStore]")
{
  ...
  DataIOCollection collection;
  SIMPLNX_RESULT_REQUIRE_VALID(collection.addIOManager(std::make_shared<HintRecordingManager>(recordedHint)));
...
TEST_CASE("BoundedRecordPageCache bulk read preserves request order across pages", "[TemporaryRecordStore]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(cache.readMany(indices, values, active));
  REQUIRE(values == std::array<uint64, 6>{11, 12, 14, 21, 22, 24});
  REQUIRE(store.readOffsets == std::vector<uint64>{0, 4});

  SIMPLNX_RESULT_REQUIRE_INVALID(cache.readMany(nonstd::span<const uint64>(indices.data(), indices.size() - 1), values, active));
  const std::array<uint64, 1> invalidIndex = {8};
  std::array<uint64, 1> invalidValue = {};
  SIMPLNX_RESULT_REQUIRE_INVALID(cache.readMany(invalidIndex, invalidValue, active));
}
...
TEST_CASE("BoundedRecordPageCache inspects and modifies a cached record in place", "[TemporaryRecordStore]")
{
  ...
  SIMPLNX_RESULT_REQUIRE_VALID(cache.inspect(1, [&inspectedValue](const uint64& value) noexcept { inspectedValue = value; }, active));
  REQUIRE(inspectedValue == 5);
  SIMPLNX_RESULT_REQUIRE_VALID(cache.modify(1, [](uint64& value) noexcept { value = 41; }, active));
  SIMPLNX_RESULT_REQUIRE_VALID(cache.flush(active));
