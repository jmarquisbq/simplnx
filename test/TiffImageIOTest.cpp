...
TEST_CASE("TiffImageIO:: row callback returns the first error", "[TiffImageIO]")
{
  ...
  auto result = imageIO.readPixelDataRows(file.path(), [&callbackCount](usize, usize, usize, std::span<const uint8>) -> Result<> {
    ++callbackCount;
    return MakeErrorResult(k_CallbackError, "Intentional TIFF row callback failure.");
  });
  ...
  REQUIRE(result.errors().front().message == "Intentional TIFF row callback failure.");
}
